...
return $aDisabled <=> $bDisabled;
    ...
    <div class="variant-block" data-variation-id="{{ $variation['id'] }}">
        <div class="d-flex align-items-center justify-content-between my-5">
            ...
            <div class="toggle-buttons" data-variation-id="{{ $variation['id'] }}">
                <button class="btn btn-link toggle-view d-flex align-items-center" type="button" data-action="show" data-variation-id="{{ $variation['id'] }}">
                    Mostra tutti
                    <svg width="17" class="ms-2" id="f91a913a-7ce6-4238-be62-40c6cb9aeb7f" data-name="Livello 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 80">
                        <title>grid-variant</title>
                        <rect width="35.45" height="35.45" style="fill:#404647" />
                        <rect x="44.55" width="35.45" height="35.45" style="fill:#404647" />
                        <rect y="44.55" width="35.45" height="35.45" style="fill:#404647" />
                        <rect x="44.55" y="44.55" width="35.45" height="35.45" style="fill:#404647" />
                    </svg>
                </button>
                <button class="btn btn-link toggle-view d-flex align-items-center d-none" type="button" data-action="hide" data-variation-id="{{ $variation['id'] }}">
                    Nascondi
                    <svg width="17" class="ms-2" id="ad941f12-1f76-45af-aba9-f8aa061814de" data-name="Livello 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 80 35.45">
            ...
            <div class="swiper-wrapper">
                ...
                <div class="swiper-slide" @if ($productshow == true && (in_array($value['id'], $variation_value_ids ?? []) || ($key === 0 && empty($variation_value_ids)))) data-selected="true" @endif>
                    <div class="gallery-item-block @if (in_array($value['id'], $variation_value_ids ?? []) || ($key === 0 && empty($variation_value_ids))) selected @endif @if ($isDisabled) disabled {{$isCompletelyDisabled}} @endif" data-value-id="{{ $value['id'] }}" @if ($isDisabled && $message) data-bs-toggle="tooltip" title="{{ $message }}" @endif>
                        ...
                        @if($value['info_description'] || $value['info_video_url'] || $value['info_image_id'] || $value['info_slider_image_ids'])
        ...
        <div class="variant-select-block d-flex align-items-center">
            <select name="variation_value_for_variation_{{ $variation['id'] }}" class="product-select form-control flex-grow-1" {{ $allDisabled ? '' : 'required' }}>
                @foreach ($variation['values'] as $value)
                @php
                $isDisabled = in_array($value['id'], $conditionEffects ?? []);
                $message = $motivationalMessages[$value['id']] ?? '';
                ...
                <option value="{{ $value['id'] }}"
                    {{ in_array($value['id'], $variation_value_ids ?? []) ? 'selected' : '' }}
                    {{ $isDisabled ? 'disabled' : '' }}
            ...
            @foreach ($variation['values'] as $value)
    @if(in_array($value['id'], $variation_value_ids) && ($value['info_description'] || $value['info_video_url'] || $value['info_image_id'] || $value['info_slider_image_ids']))
        
            <span class="info-icon info-icon-select ms-2" style="width: 32px;" data-value-id="{{ $value['id'] }}">
                <svg xmlns="http://www.w3.org/2000/svg" class="info-svg" viewBox="0 0 32 32" width="32px" height="32px">
                    <path d="M 16 3 C 8.832031 3 3 8.832031 3 16 C 3 23.167969 8.832031 29 16 29 C 23.167969 29 29 23.167969 29 16 C 29 8.832031 23.167969 3 16 3 Z M 16 5 C 22.085938 5 27 9.914063 27 16 C 27 22.085938 22.085938 27 16 27 C 9.914063 27 5 22.085938 5 16 C 5 9.914063 9.914063 5 16 5 Z M 15 10 L 15 12 L 17 12 L 17 10 Z M 15 14 L 15 22 L 17 22 L 17 14 Z" />
                </svg>
            </span>
        ...
        <div id="modal-container-{{ $variation['id'] }}"></div>
