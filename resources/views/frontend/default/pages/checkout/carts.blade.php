        ...
        <div class="row g-4">
            <div class="col-xl-7">
                <div class="voucher-box py-7 px-5 position-relative z-1 overflow-hidden bg-white rounded mt-4">
                    <img src="{{ staticAsset('frontend/default/assets/img/shapes/circle-half.png') }}" alt="circle shape" class="position-absolute end-0 top-0 z--1">
                    ...
                    <form class="d-flex align-items-center coupon-form">
                        ...
                        <input type="text" name="code" placeholder="{{ localize('Enter Your Coupon Code') }}" class="theme-input w-100 coupon-input" @if (isset($_COOKIE['coupon_code'])) value="{{ $_COOKIE['coupon_code'] }}" disabled @endif required>

                        @if (isset($_COOKIE['coupon_code']))
                        <button type="submit" class="btn btn-secondary flex-shrink-0 apply-coupon-btn d-none px-4">{{ localize('Apply Coupon') }}</button>
                        <button type="button" class="btn btn-secondary flex-shrink-0 clear-coupon-btn"><i class="fas fa-close"></i></button>
                        @else
                        <button type="submit" class="btn btn-secondary flex-shrink-0 apply-coupon-btn px-4">{{ localize('Apply Coupon') }}</button>
                        <button type="button" class="btn btn-secondary flex-shrink-0 clear-coupon-btn d-none"><i class="fas fa-close"></i></button>
                        @endif
                    </form>
                    <!-- coupon form -->
            ...
            <div class="col-xl-5">
                <div class="cart-summery bg-white rounded-2 pt-4 pb-7 px-5 mt-4">
                        ...
                        <tr class="coupon-discount-wrapper {{ getCoupon() == '' ? 'd-none' : '' }}">
                            ...
                            <td class="py-3">
                                ...
                                <h5 class="mb-0 text-end coupon-discount-price">
                                    ...
                                    @else
                                        @if($couponDetails && $couponDetails->is_free_shipping)
                                            {{ localize('Spedizione gratuita') }}
                                        @endif
                                    @endif
                                </h5>
                            </td>
                    ...
                    <div class="btns-group d-flex flex-wrap gap-3">
                        ...
                        <a href="{{ route('home') }}" class="btn btn-outline-secondary border-secondary btn-md rounded-1">{{ localize('Continue Shopping') }}</a>

                        <a href="{{ route('checkout.proceed') }}" type="submit" class="btn btn-primary btn-md rounded-1">{{ localize('Checkout') }}</a>
                    </div>
                </div>
            </div>
