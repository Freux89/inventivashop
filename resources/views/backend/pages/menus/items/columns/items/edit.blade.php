                        ...
                        <form action="{{ route('admin.menu-column-items.update', $menuColumnItem->id) }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="mb-3">
                                ...
                                <select class="form-select @error('link_type') is-invalid @enderror" id="link_type" name="link_type">
                                    ...
                                    <option value="url" {{ old('link_type', $menuColumnItem->link_type) == 'url' ? 'selected' : '' }}>{{ localize('URL Personalizzato') }}</option>
                                    <option value="product" {{ old('link_type', $menuColumnItem->link_type) == 'product' ? 'selected' : '' }}>{{ localize('Prodotto') }}</option>
                                    <option value="category" {{ old('link_type', $menuColumnItem->link_type) == 'category' ? 'selected' : '' }}>{{ localize('Categoria') }}</option>
                                </select>
                                @error('link_type')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                            ...
                            <div class="mb-3 link-type-field" id="url_field" style="display: none;">
                                ...
                                <input type="text" class="form-control @error('url') is-invalid @enderror" id="url" name="url" value="{{ old('url', $menuColumnItem->url) }}">
                                @error('url')
                                <span class="invalid-feedback" role="alert">
                                    <strong>{{ $message }}</strong>
                                </span>
                            ...
                            <div class="mb-3 link-type-field" id="product_field" style="display: none;">
                                ...
                                <select class="form-select @error('product_id') is-invalid @enderror" id="product_id" name="product_id">
                                    ...
                                    <option value="{{ $product->id }}" {{ old('product_id', $menuColumnItem->product_id) == $product->id ? 'selected' : '' }}>{{ $product->name }}</option>
                                    @endforeach
                                </select>
                                @error('product_id')
                                <span class="invalid-feedback" role="alert">
                            ...
                            <div class="mb-3 link-type-field" id="category_field" style="display: none;">
                                ...
                                <select class="form-select @error('category_id') is-invalid @enderror" id="category_id" name="category_id">
                                    ...
                                    <option value="{{ $category->id }}" {{ old('category_id', $menuColumnItem->category_id) == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                                @error('category_id')
                                <span class="invalid-feedback" role="alert">
